...
> **⚠️ BU BELGE GEÇERSİZDİR — YERİNE `denetci-proje-bilgi-formu.md` ESAS ALINIR (Eylül 2026).**
> Hakem, alıcı, kapsam ve ikame merceklerinden yapılan değerlendirme sonucunda proje yalınlaştırılmış ve teknoparkın gerçek Proje Bilgi Formu başlıklarına taşınmıştır. Bu belgedeki şu unsurlar **artık geçersizdir:** (1) CBAM/SKDM ayağı — olgu hatası nedeniyle tamamen çıkarıldı (CBAM Ek-I listesinde seramik yoktur; beyan yükümlüsü AB'deki ithalatçıdır); (2) AS-5 (yerel dil modeli damıtma) araştırma sorusu — rutin mühendislik sayıldığı için kesildi, hazır model + şema-zorlamalı çözümleme olarak geliştirme kalemine indi; (3) AS-4 — AS-2 içine eritildi; (4) plan rakamları — 10 ay/28-30 adam-ay/9,0 M TL yerine **12 ay/25 adam-ay/7,5 M TL**; (5) altı ürün modülü — üç ürün satırı ve iki satırlık sipariş formuna indirildi. Belgenin hâlâ geçerli olan kısımları (deterministik çekirdek ilkesi, kapalı devre/CPU-yalnız mimari, mutabakat ve mühürlü baz tasarımı) güncel forma taşınmıştır. Teknik arka plan olarak saklanmaktadır.
