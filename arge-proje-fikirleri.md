...
> **#5 için teknopark proje başvurusu hazırlandı:** form-hazır güncel sürüm [`argus-teknopark-basvuru-formu.md`](argus-teknopark-basvuru-formu.md) (genişletilmiş taslak: [`teknopark-proje-basvurusu-regtech.md`](teknopark-proje-basvurusu-regtech.md)). **Mimari kararları:** (1) Asıl katman deterministik hesap motorlarıdır — Argelog bünyesinde önceden geliştirilmiş teşvik hesap çekirdeği (Bakanlık doğrulamalı, 96/96 çapa testi) başlangıç varlığı olarak devralınır; LLM yalnızca dar görevlerde (mevzuat izleme özeti, kanıt eşleştirme, rapor dili) ve insan onaylı çalışır. (2) Tüm YZ işleme yereldir — KVKK gereği bordro verisi kurum dışına çıkamaz, harici LLM API kullanılmaz. Güncel plan: **10 ay / 25 adam-ay / ~7,95 M TL**. **Pivot (karar):** ürün, kapalı devre ortamdaki sıradan kişisel bilgisayarlarda **CPU-yalnız** çalışan masaüstü uygulama olarak teslim edilir — 1-4B sınıfı damıtılmış modeller + GGUF 4-bit + llama.cpp; GPU sunucu yatırımı kaldırıldı, imzalı offline mevzuat güncelleme paketi eklendi; SMMM/YMM ofisleri ve savunma kapalı ağları hedef pazara katıldı. Aynı ilkeler #1 Copilot için de geçerli olacaktır. **GÜNCEL KARAR (Eylül 2026, yalınlaştırma sonrası):** Proje **Denetci** adıyla, teknoparkın gerçek Proje Bilgi Formu başlıklarına taşınmış ve yalınlaştırılmıştır — güncel ve geçerli belge: [`denetci-proje-bilgi-formu.md`](denetci-proje-bilgi-formu.md). Kapsam: 5746 ve kardeş rejimi 4691 (CBAM/SKDM ayağı olgu hatası nedeniyle tamamen çıkarıldı); üç araştırma sorusu (zaman-farkındalıklı kural sürümleme, fark kök-neden teşhisi, rejimden bağımsızlık ölçümü); 12 ay / 25 adam-ay / 7,5 M TL; üç ürün satırı. Aşağıdaki v1 analizleri tarihsel kayıt olarak durmaktadır. **Ar-Ge/market-fit çekirdeği:** geçmiş beyanların yüklenip doğrulanması, tutarlılık denetimi ve sisteme baz oluşturması "BAZ Hattı" alt sistemi olarak ayrıntılı tasarlandı — bkz. [`argus-baz-hatti-tasarimi.md`](argus-baz-hatti-tasarimi.md) (veri talep listesi, A0–A7 hattı, K1–K14 kuralları, S1–S5 fark sınıfları, AS-1..AS-4 araştırma soruları, kapılı pilot akışı ve keşif görüşmesi soruları). **Hızlı gelir planı:** proje bitişini beklemeden hizmet-önce gelir modeli — bkz. [`argus-gelir-plani.md`](argus-gelir-plani.md) (katma değer tezi ve TL değer hesabı, 6 basamaklı gelir merdiveni, paketler/fiyatlar, 30/60/90 gün planı, 3 senaryolu nakit projeksiyonu, yasal sınırlar ve kırmızı çizgiler).
